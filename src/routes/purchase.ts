        ...
        switch (data.type) {
            case 'INIT':
                try {
                    ...
                    if (purchaseResult.rows.length === 0 || purchaseResult.rows[0].user_id !== sessionState.userId) return ws.close(1011, '购买记录未找到。');
                    if (purchaseResult.rows[0].status !== 'paid') return ws.close(1011, `购买状态不是 'paid'。`);
